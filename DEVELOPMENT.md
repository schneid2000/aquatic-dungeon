...
- Player Mechanics
  - Added the player's inventory. It contains three special equipment slots and six normal slots.
  - When an enemy is defeated, an item is dropped that the player can pick up.
  - The player can equip a Weapon and increase the damage they do to enemies
  - The player can equip Armor and decrease the damage done to them
  - I also included a magic slot, which increases Magic Damage, but I did not have the time to implement that feature.
  - The player can click an item in the inventory to select it, and click on a second slot to move the item.
  - Only items of a specified type can be moved to the three equipment slots; Weapon, Armor, and Magic items respectively.
  - Finally, health potions can be clicked twice in the inventory to restore some of the player's health.

- Item Mechanics
  - An Item will randomly be one of the four types when it is generated (Weapon, Magic, Armor, Health Potion)
  - If it is one of the three special equipment types, it will receive a random name based on several presets
  - The item will also have a random type-based image based on several images
  - As aforementioned Items are entities with an InventorySlot component
  - The player object manages which slots are full, and the Items manage which slot they are in.

- Boss Mechanics
  - There is now a boss spawn tile in the boss room. When the player walks on the spawn tile, the room is walled off and a boss fight begins.
  - A boss is randomly selected, though I have only implemented one; the Cuttlefish.
  - The boss has its own health bar that displays in the top right corner of the screen, along with its name.
  - The Cuttlefish has one attack where it summons a tendril that spawns in a random location in the boss room.
  - After 2 turns the tendrils breaks through the floor and does damage to the player if they are standing on it.
  - Additionally the player cannot walk onto tiles with a tendril on its third turn.
  - Every turn the Cuttlefish will summon a new tendril. 

- Winning and Losing Mechanics
  - Now when the player runs out of health, a "You lose!" message is displayed.
  - Additionally when the player defeats the boss, a "You win!" message is displayed.
  - After the player wins or loses, pressing any key will exit the game.

- Obstacles
  - I recently overloaded the equality operator for my coordinate object, and I should have done it much earlier. Comparing coordinates took up a lot of space.
  - Trying to reduce repeating methods and code was hard.
  - It is kind of a struggle to add more tests, simply because of how much can be tested.

- Progress
  - The player can pick up items from enemies when they are defeated, and manage the items in their inventory.
  - The player can equip Weapons or Armor to increase their offensive or defensive capabilities
  - The player can fight and defeat a boss
  - The player can die and lose the game, or beat the boss and win the game
